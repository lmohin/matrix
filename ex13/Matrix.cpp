...
bool	Matrix::rowIsNull(size_t const &row) const
{
	for (size_t col = 0; col != _cols; col++)
	{
		if ((*this)(row, col) != 0)
...
size_t	Matrix::rank(void) const
{
	Matrix	reductedMatrix = gaussianElimination();

	size_t	row = 0;
	while (row != _rows && !reductedMatrix.rowIsNull(row))
		row++;
	return (row);
}
